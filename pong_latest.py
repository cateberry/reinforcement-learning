...
class DDQN(nn.Module):
    """
    Double Deep Q-Network CNN model definition.
    """
...
# Config for wandb
wandb.init(project="reinforcement-learning-coursework", name="ddqn-pong",
...
policy_net = DDQN(input_shape, n_actions, learning_rate).to(device)
target_net = DDQN(input_shape, n_actions, learning_rate).to(device)
...
# resume = False
# model_name = 'DDQN' + '_40_200503-1956'# fill in file path for checkpoint
...
for e in range(num_eps):
    ...
    if e % 50 == 0:
        model_name = 'DDQN_' + str(e) + "_" + str(datetime.datetime.now().strftime("%y%m%d-%H%M"))
        save_name = 'checkpoints/' + model_name
